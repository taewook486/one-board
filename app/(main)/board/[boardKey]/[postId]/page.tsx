...
export default function PostDetailPage({
  ...
  params: { boardKey: string; postId: string };
}) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const postResponse = await fetch(
          `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/posts/${params.postId}`,
          { cache: 'no-store' }
        );
        ...
        if (postData.success) {
          ...
          router.push(`/board/${params.boardKey}`);
          return;
        }

        // Fetch comments
        const commentsResponse = await fetch(
          `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/comments?postId=${params.postId}&tree=true`,
          { cache: 'no-store' }
        );
    ...
    loadData();
  }, [params.postId, params.boardKey, router]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push(`/board/${board?.boardKey || params.boardKey}`);
      } else {
            ...
            <div className="flex items-start justify-between">
              ...
              {canEdit && (
                <div className="flex gap-2">
                  <Link
                    href={`/write?postId=${post.id}&board=${params.boardKey}`}
            ...
            <div className="flex items-center justify-between">
              ...
              {adjacent && (
                <div className="flex gap-4 text-sm">
                  {adjacent.previous && (
                    <Link
                      href={`/board/${params.boardKey}/${adjacent.previous.id}`}
                  ...
                  {adjacent.next && (
                    <Link
                      href={`/board/${params.boardKey}/${adjacent.next.id}`}
                      className="text-gray-600 hover:text-blue-600"
                    >
                      다음글 →
                    </Link>
